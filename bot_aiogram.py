...
# ==================== ПОДКЛЮЧЕНИЕ К БД ====================
_QUOTA_PHRASES = ("quota", "exceeded the compute", "upgrade your plan", "compute time")
...
def _db():
    ...
    for _attempt in range(3):
        try:
            ...
            # Ошибки квоты/лимита провайдера — повтор бессмысленен
            if any(p in err_lower for p in _QUOTA_PHRASES):
                raise RuntimeError(
                    f"❌ База данных недоступна: исчерпан лимит вычислительного времени.\n"
                    f"Обновите тариф Neon или смените DATABASE_URL на новый проект.\n"
                    f"Оригинальная ошибка: {e}"
                ) from e
            # Временные ошибки SSL/сети — повторяем
            if "ssl" in err_lower or "connection" in err_lower:
                _time.sleep(1)
                continue
            raise
    raise last_err
...
async def main():
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f"ОШИБКА БАЗЫ ДАННЫХ — бот не может запуститься:")
        print(str(db_err))
        print(f"{'='*60}")
        print("Решение: замените DATABASE_URL на рабочую базу данных")
        print("  • Создайте новый проект на https://neon.tech")
        print("  • Или используйте Supabase / Railway")
        print("  • Обновите переменную DATABASE_URL в настройках Render")
        print(f"{'='*60}\n")
        raise SystemExit(1)
